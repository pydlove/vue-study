<template>
    <div>
        <el-dialog class="aioc-forget wow rollIn"
            ...
            <!--邮箱-->
            <div v-if="showForgetPage" class="forget-main">
                <div class="forget-logo">
                    <img :src="logo" alt="logo图">
                    <span>梧桐圈</span>
                </div>
                <div class="forget-divider">
                    <el-divider class="wdi-400" content-position="center">使用手机号/邮箱重置密码</el-divider>
                </div>
                <div class="forget-mailboxform">
                    ...
                    <el-input :class="pwdErrorMsg.length > 0 ? 'bor-red wdi-400' : 'wdi-400'" show-password
                              placeholder="重置密码，密码不少于8位" v-model="form.password"
                              @blur="checkPwdFormat"
                    ></el-input>
                    <div class="pwdErrormsg">{{pwdErrorMsg}}</div>
                    <el-button class="mti-10 wdi-400" type="primary" @click="submit">重 置 密 码</el-button>
                </div>
                <div class="forget-ba mt-20">
    ...
    export default {
        ...
        methods: {
            ...
            sendVerificationCode() {
                if(this.forgetType == "mail") {
                    this.sendVerificationCodeToMail();
                }
                else if (this.forgetType == "phone") {
            ...
            async sendVerificationCodeToMail() {
                ...
                if (!this.timer) {
                    ...
                    let data = await this.$aiorequest(this.$aiocUrl.cpcm_service_v1_forget_mail_sendVerificationCode, params, "POST")
                    if (data.code === 200) {
            ...
            async sendVerificationCodeToPhone() {
                ...
                if (!this.timer) {
                    ...
                    let data = await this.$aiorequest(this.$aiocUrl.cpcm_service_v1_forget_phone_sendVerificationCode, params, "POST")
                    if (data.code === 200) {
                        if(data.data == "SUCCESS") {
                            this.errorMsg = "";
                            this.count = TIME_COUNT;
            ...
            async mailForget() {
                ...
                let data = await this.$aiorequest(this.$aiocUrl.cpcm_service_v1_forget_mail, params, "POST");
                this.disableSubmit = false;
                if (data.code === 200) {
                    if(data.data == "SUCCESS") {
                        this.close();
                        this.$promptMsg("密码重置成功，请登录", "success");
                        var that = this;
            ...
            async phoneForget() {
                ...
                params.append("password", this.$md5(this.form.password));
                params.append("verificationCode", this.form.verificationCode);
                let data = await this.$aiorequest(this.$aiocUrl.cpcm_service_v1_forget_phone, params, "POST");
                this.disableSubmit = false;
                if (data.code === 200) {
                    if(data.data == "SUCCESS") {
                        this.close();
...
<style scoped>
    ...
    @media screen and (min-width: 1529px) {
        ...
        .forget-tba a:nth-of-type(1){
            color: #ec414d;
        }
        .forget-tba a:nth-of-type(1):hover {
            text-decoration: underline;
        }
        .forget-ba a:nth-of-type(1) {
            color: #ec414d;
        }
        .forget-ba a:nth-of-type(1):hover {
            text-decoration: underline;
        }
        .verification-main {
            display: flex;
...
<style>
    @media screen and (min-width: 1529px) {
        .aioc-forget .el-dialog__body {
            padding:0px 20px 30px 20px;
            display: flex;
            justify-content: center;
        }
        .forget-type .el-card:nth-of-type(1) > .el-card__body {
            background: #6e2e22;
            color: #ffffff;
        }
        .forget-type .el-card:nth-of-type(2) > .el-card__body {
            background: #163e7a;
            color: #ffffff;
        }
        .forget-divider .el-divider__text {
